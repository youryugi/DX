...
# 数据准备
labels = ['α=0\n(一番短いルート) ', 'α=0.1', 'α=0.2', 'α = 0.3 to 0.9 \n(step 0.1)', 'α=1']
labels = ['', '', '', '', '']
...
# 画阳光段（底部红色）
ax.bar(x, sunny_lengths, color='red', label='日向の長さ')

# 画阴影段（顶部绿色）
ax.bar(x, shadow_lengths, bottom=sunny_lengths, color='green', label='日陰の長さ')
# 添加数值标签
# 添加内部数值标签
...
# 设置标题和标签
ax.set_ylabel('距離 (m)', fontsize=bigfontsize)
