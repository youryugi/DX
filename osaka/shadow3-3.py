...
# 6. 根据太阳角度和高度生成阴影
def generate_shadow_2d(geometry, height):
    """
    生成建筑物在地面上的二维阴影。

    参数:
    - geometry: 建筑物的二维几何 (Polygon 或 MultiPolygon)
    - height: 建筑物高度 (float)

    返回:
    - 投影阴影的 Polygon 或 MultiPolygon
    """
    if sun_vector[2] <= 0:
    ...
    if geometry.geom_type in ['Polygon', 'MultiPolygon']:
        ...
        for poly in polygons:
            ...
            shadow_coords = [
                (
                    x - shadow_length * sun_vector[0],  # 根据太阳方向计算投影
                    y - shadow_length * sun_vector[1]
                )
                for x, y in base_coords
            ]

            # 创建阴影多边形
            full_coords = base_coords + shadow_coords[::-1]  # 将建筑物底部与阴影顶点连接
            if full_coords[0] != full_coords[-1]:
                full_coords.append(full_coords[0])  # 确保多边形闭合

            try:
                shadow_polygon = Polygon(full_coords)
                if not shadow_polygon.is_valid:
                    shadow_polygon = shadow_polygon.buffer(0)  # 修复无效几何体
                shadow_polygons.append(shadow_polygon)
            except Exception as e:
                print(f"创建阴影多边形失败: {e}")
                continue

        # 如果有多个阴影多边形，返回 MultiPolygon，否则返回单一 Polygon
        if shadow_polygons:
