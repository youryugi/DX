...
def update_cool_route(coef, start_time, sample_interval=300):
    ...
    return route
def compute_pareto_routes(start_dt, coef=0.0, sample_interval=300):
    """
    返回 routes, front  两个列表
    routes : [GeoDataFrame, ...]   每条 Pareto 路径
    front  : [(dist, sun), ...]    对应指标，用来画折线
    """
    speed = 10 * 1000 / 3600      # m/s

    # -------- 1) 起终点 --------
    if origin_point_wgs84 is None or destination_point_wgs84 is None:
        print("请先点选起终点！"); return [], []
    s = ox.distance.nearest_nodes(G, X=origin_point_wgs84[1], Y=origin_point_wgs84[0])
    g = ox.distance.nearest_nodes(G, X=destination_point_wgs84[1], Y=destination_point_wgs84[0])

    # -------- 2) label 定义 --------
    from collections import defaultdict
    import heapq
    Label = tuple   # (dist, sun, t, node, pred_node, pred_t)

    # labels_at[node] = [Label, ...]
    labels_at = defaultdict(list)
    pq = []  # (key, Label)  key = dist + heuristic

    def heuristic(n):
        lat, lon = G.nodes[n]['y'], G.nodes[n]['x']
        glat, glon = G.nodes[g]['y'], G.nodes[g]['x']
        hdist = ox.distance.euclidean_dist_vec(lat, lon, glat, glon)
        # 这里可加一个乐观假设：全阴影 ratio=0
        return hdist

    # 起点标签
    lb0 = (0.0, 0.0, 0.0, s, None, None)
    labels_at[s].append(lb0)
    heapq.heappush(pq, (heuristic(s), lb0))

    # -------- 3) 主循环 --------
    while pq:
        _, lb = heapq.heappop(pq)
        dist, sun, tcur, u, pu, pt = lb

        # 若此标签已被支配，则跳过
        if any((d <= dist and s_ <= sun) for (d, s_, *_)
               in labels_at[u] if (d, s_, *_ ) != lb):
            continue

        # 到达终点就不再扩展（仍需保留其他可能标签）
        if u == g: 
            continue

        arr_dt = start_dt + timedelta(seconds=tcur)
        ...
        for v, edict in G[u].items():
            for k in edict:
                ...
                # 计算阴影比例（积分同你原函数）
                tmp_dt, remain, sun_len = arr_dt, dur, 0.0
                while remain > 0:
                    dt = min(sample_interval, remain)
                    mid = tmp_dt + timedelta(seconds=dt/2)
                    nt  = find_nearest_time(time_to_union.keys(), mid)
                    ratio = precomputed.get((u, v, k, nt), 0.0)
                    sun_len += length * (1 - ratio) * (dt/dur)
                    tmp_dt += timedelta(seconds=dt); remain -= dt

                new_lb = (dist+length, sun+sun_len, tcur+dur, v, u, tcur)

                # ---- 支配检查 ----
                dominated = False
                for od, os, *_ in labels_at[v]:
                    if od <= new_lb[0] and os <= new_lb[1]:
                        dominated = True; break
                if dominated: continue

                # 移除被新标签支配的旧标签
                labels_at[v] = [(od, os, *rest) for (od, os, *rest)
                                in labels_at[v]
                                if not (new_lb[0] <= od and new_lb[1] <= os)]

                labels_at[v].append(new_lb)
                heapq.heappush(pq, (new_lb[0] + heuristic(v), new_lb))

    # -------- 4) 回溯得到所有路径 --------
    routes, front = [], []
    for dist, sun, tcur, node, pred_node, pred_t in labels_at[g]:
        # 回溯
        path_nodes = [g]; n, nt = pred_node, pred_t
        while n is not None:
            path_nodes.append(n)
            # 找到 n 的上一个标签
            n_lb = next(lb for lb in labels_at[n] if lb[2] == nt)
            n, nt = n_lb[4], n_lb[5]
        path_nodes.reverse()

        edges = []
        for i in range(len(path_nodes)-1):
            uu, vv = path_nodes[i], path_nodes[i+1]
            k = 0 if (uu, vv, 0) in gdf_edges.index else \
                next(kk for kk in G[uu][vv] if (uu, vv, kk) in gdf_edges.index)
            edges.append((uu, vv, k))
        routes.append(gdf_edges.loc[edges].copy())
        front.append((dist, sun))

    # 距离升序排序，使绘图更有序
    front, routes = zip(*sorted(zip(front, routes), key=lambda x: x[0][0]))
    return list(routes), list(front)
...
def plot_pareto_on_map(routes, front, cmap='plasma'):
    import matplotlib.cm as cm
    colormap = plt.get_cmap(cmap, len(routes))
    for idx, (gdf, (dist, sun)) in enumerate(zip(routes, front)):
        shade_ratio = 1 - sun/dist if dist > 0 else 0
        color = colormap(idx)
        gdf.plot(ax=ax, linewidth=2, color=color,
                 label=f'R{idx+1}  距{dist:.0f}m  阳{sun:.0f}m  阴{shade_ratio*100:.0f}%')
...
def generate_pareto(event):
    routes, front = compute_pareto_routes(start_time, sample_interval=60)
    ax_scatter = fig.add_axes([0.75, 0.3, 0.22, 0.25])
    dists  = [d for d, s in front]
    suns   = [s for d, s in front]
    ax_scatter.plot(dists, suns, marker='o')
    ax_scatter.set_xlabel('Total distance (m)')
    ax_scatter.set_ylabel('Sunny distance (m)')
    ax_scatter.set_title('Pareto front')
    if not routes:
        return
    plot_pareto_on_map(routes[:10], front[:10])  # 只画前10条
    ax_scatter.clear()
    ax_scatter.plot([d for d,s in front], [s for d,s in front], '-o')
    ax_scatter.set_xlabel('Dist'); ax_scatter.set_ylabel('Sun')
    plt.draw()

ax_button_pareto = plt.axes([0.65, 0.05, 0.1, 0.075])
button_pareto = Button(ax_button_pareto, 'Pareto')
button_pareto.on_clicked(generate_pareto)
