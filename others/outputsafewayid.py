...
# 找到所有除了给定way_ids之外的way id
all_other_ways = [way_id for way_id in way_id_to_element if way_id not in way_ids]

# 将其他ways的id信息转换为DataFrame
other_ways_df = pd.DataFrame(all_other_ways, columns=['way_id'])

# 输出结果到新的xlsx文件，使用xlsxwriter加快速度
output_other_ways_file = 'output_other_ways_with_ids_without_acc.xlsx'
