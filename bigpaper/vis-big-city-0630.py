...
date_time = datetime(2025, 7, 5, 9, 0, tzinfo=timezone(timedelta(hours=9)))
...
plt.rcParams['font.size'] = bigfont
...
legend_handles = [
    ...
    Patch(facecolor='gray', label='Shade'),
]
plt.title("Roads, buildings, and shade     ↑North", fontsize=bigfont)
plt.xlabel("X (m)", fontsize=bigfont)
plt.ylabel("Y (m)", fontsize=bigfont)
plt.legend(handles=legend_handles, loc='upper right', fontsize=bigfont)
plt.grid(True)
