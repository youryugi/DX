# -*- coding: utf-8 -*-
"""
Shortest‑path edge‑frequency analysis between **office buildings** (usage 401)
and the nearest railway station ("駅") in a given area.

Revision highlights
-------------------
* **Usage code** switched from 411/412 (residential) to **401 (office)**.
* Removed apartment‑specific weighting; every office → station trip counts as 1.
* Added `USAGE_COLOR_MAP["401"]` (blue) and updated legends/titles/file names.
* The rest of the workflow (filter → graph download → path search → plot → save) remains unchanged.
"""

import os
import osmnx as ox
import networkx as nx
import geopandas as gpd
import matplotlib.pyplot as plt
import pandas as pd
from collections import Counter
from matplotlib.patches import Patch

# --------------------------------------------------
# 1) Configurable parameters
# --------------------------------------------------
PADDING_DEG = 0.001        # ~110 m latitude‑wise
PROJECTED_CRS = 6669       # JGD2011 / Japan Plane Rectangular CS II (Kobe‑Osaka‑Kyoto)

# Colours
DEFAULT_COLOR = "#d9d9d9"  # light grey for unclassified
USAGE_COLOR_MAP = {
    "401": "#377eb8",   # blue   – Office building
    "station": "#ff7f00"  # orange – Station (駅)
}

# --------------------------------------------------
# 2) Read building footprints (CityGML / PLATEAU)
# --------------------------------------------------
print("1. 正在读取建筑数据…")
BLDG_GML_FILES = [
    r"bldg/51357451_bldg_6697_op.gml",
    # 其他文件按需打开
]

building_gdf = gpd.GeoDataFrame(
    pd.concat([gpd.read_file(f) for f in BLDG_GML_FILES], ignore_index=True)
)

# Filter: usage 401 OR name contains "駅"
print("   按 usage 与名称过滤…")
is_office = building_gdf["usage"].astype(str).str.startswith("401")
is_station = building_gdf["name"].fillna("").str.contains("駅", regex=False)

building_gdf = building_gdf[is_office | is_station].copy()

# Colours
building_gdf["color"] = DEFAULT_COLOR
building_gdf.loc[is_office, "color"] = USAGE_COLOR_MAP["401"]
building_gdf.loc[is_station, "color"] = USAGE_COLOR_MAP["station"]

print("   过滤后建筑总数:", len(building_gdf))
print("   usage 唯一值:", building_gdf["usage"].unique())

# --------------------------------------------------
# 3) Download / cache drive network inside bounding box
# --------------------------------------------------
print("2. 计算建筑物范围并下载对应路网…")
if building_gdf.crs is None:
    building_gdf.set_crs(epsg=4326, inplace=True)
elif building_gdf.crs.to_epsg() != 4326:
    building_gdf = building_gdf.to_crs(epsg=4326)

minx, miny, maxx, maxy = building_gdf.total_bounds
minx -= PADDING_DEG
miny -= PADDING_DEG
maxx += PADDING_DEG
maxy += PADDING_DEG
print(f"   路网下载范围: ({miny:.5f}, {maxy:.5f}, {minx:.5f}, {maxx:.5f})")

bbox_str = f"{miny:.5f}_{maxy:.5f}_{minx:.5f}_{maxx:.5f}"
GRAPHML_PATH = f"cached_network_{bbox_str}.graphml"

if os.path.exists(GRAPHML_PATH):
    print(f"   已检测到缓存文件 {GRAPHML_PATH}，直接读取…")
    G = ox.load_graphml(GRAPHML_PATH)
else:
    print("   无缓存，开始下载路网 (network_type='drive')…")
    G = ox.graph_from_bbox(maxy, miny, maxx, minx, network_type="drive")
    ox.save_graphml(G, GRAPHML_PATH)
    print(f"   路网已保存到 {GRAPHML_PATH}")

print("   路网加载完成: 节点", len(G.nodes), "边", len(G.edges))

# --------------------------------------------------
# 4) Prepare centroid lists (projected CRS for distance calcs)
# --------------------------------------------------
print("3. 计算 office / 駅 的质心…")
if building_gdf.crs.to_epsg() != PROJECTED_CRS:
    building_gdf_proj = building_gdf.to_crs(epsg=PROJECTED_CRS)
else:
    building_gdf_proj = building_gdf

bldg_office = building_gdf_proj[is_office]
bldg_station = building_gdf_proj[is_station]

if bldg_office.empty or bldg_station.empty:
    raise RuntimeError("未找到 401 或 駅 建筑，无法继续。")

bldg_office_cent = bldg_office.geometry.centroid
bldg_station_cent = bldg_station.geometry.centroid

# --------------------------------------------------
# 5) Accumulate shortest paths
# --------------------------------------------------
print("4. 正在计算最短路径并统计边频率…")
edge_counter: Counter = Counter()
G_proj = ox.project_graph(G, to_crs=f"EPSG:{PROJECTED_CRS}")

for i, orig_pt in enumerate(bldg_office_cent):
    dest_idx = bldg_station_cent.distance(orig_pt).idxmin()
    dest_pt = bldg_station_cent.loc[dest_idx]

    # WGS84 坐标用于最近节点搜索
    orig_wgs = gpd.GeoSeries([orig_pt], crs=PROJECTED_CRS).to_crs(epsg=4326).iloc[0]
    dest_wgs = gpd.GeoSeries([dest_pt], crs=PROJECTED_CRS).to_crs(epsg=4326).iloc[0]

    try:
        orig_node = ox.distance.nearest_nodes(G, X=orig_wgs.x, Y=orig_wgs.y)
        dest_node = ox.distance.nearest_nodes(G, X=dest_wgs.x, Y=dest_wgs.y)
        path = nx.shortest_path(G, orig_node, dest_node, weight="length")
        for u, v in zip(path[:-1], path[1:]):
            edge = (u, v) if G.has_edge(u, v) else (v, u)
            edge_counter[edge] += 1
    except nx.NetworkXNoPath:
        print(f"   * 无路可达: office {i+1}")
    if (i + 1) % 100 == 0 or (i + 1) == len(bldg_office_cent):
        print(f"   已处理 {i+1}/{len(bldg_office_cent)} 条记录")

# --------------------------------------------------
# 6) Visualisation
# --------------------------------------------------
print("5. 绘图…")
fig, ax = plt.subplots(figsize=(12, 8))
ox.plot_graph(G_proj, ax=ax, show=False, close=False,
              edge_color="lightgray", edge_linewidth=0.5, node_size=0)

building_gdf_proj.plot(ax=ax, color=building_gdf_proj["color"],
                       edgecolor=None, linewidth=0.3, alpha=0.9)

if edge_counter:
    max_freq = max(edge_counter.values())
    for (u, v), f in edge_counter.items():
        data = G_proj.get_edge_data(u, v)
        geom = data[0].get("geometry") if data else None
        xs, ys = geom.xy if geom is not None else (
            [G_proj.nodes[u]["x"], G_proj.nodes[v]["x"]],
            [G_proj.nodes[u]["y"], G_proj.nodes[v]["y"]],
        )
        ax.plot(xs, ys, color="red", linewidth=1 + 4 * f / max_freq, alpha=0.8)

legend_handles = [
    Patch(facecolor=USAGE_COLOR_MAP["401"], label="Office"),
    Patch(facecolor=USAGE_COLOR_MAP["station"], label="Station"),
]
ax.legend(handles=legend_handles, loc="upper right", fontsize=12)
ax.set_title("Edge frequency: Office → Station", fontsize=16)
plt.tight_layout()
plt.savefig(f"edge_freq_office-station_{bbox_str.replace('_', '-')}.png", dpi=300)
plt.show()

# --------------------------------------------------
# 7) Save edge frequency
# --------------------------------------------------
print("6. 保存边频率数据…")
records = []
for (u, v), freq in edge_counter.items():
    data = G_proj.get_edge_data(u, v)
    geom = data[0].get("geometry") if data else None
    records.append({"u": u, "v": v, "freq": freq, "geometry": geom})

freq_gdf = gpd.GeoDataFrame(records, geometry="geometry", crs=f"EPSG:{PROJECTED_CRS}")

tag = bbox_str.replace("_", "-")
freq_gdf.to_file(f"edge_freq_office-station_{tag}.gpkg", layer="edge_freq", driver="GPKG")
freq_gdf.drop(columns="geometry").to_csv(f"edge_freq_office-station_{tag}.csv", index=False)
print("   完成！")
