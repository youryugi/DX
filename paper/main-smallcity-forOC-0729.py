...
# -------------------------
# 自定义复现起点和终点坐标 (EPSG:6669)
# 如果不想用交互式，可以直接填这两个
# -------------------------
use_fixed_points = False   # True=固定复现，False=手动点击 还需要完善

start_x, start_y = 34.62781943213629, 135.5227783056634
end_x, end_y = 34.63280878029725, 135.52086277042565
...
date_time = datetime(2025, 9, 22, 15, 00, tzinfo=timezone(timedelta(hours=9)))
...
#plt.legend(handles=legend_handles, loc='upper right',fontsize=bigfontsize)
...
# 计算一条路径在阳光/日荫中的长度（单位：米）
def split_sun_shadow_length(line_geoms, shadow_union):
    sunny = 0.0
    shaded = 0.0
    for g in line_geoms:
        L = g.length
        inter = g.intersection(shadow_union)
        sh = inter.length if not inter.is_empty else 0.0
        shaded += sh
        # 防止浮点误差导致出现负值
        sunny += max(L - sh, 0.0)
    return sunny, shaded
...
def generate_path(event):
    ...
    # —— 新增：打印最短路径的阳光/日荫长度 ——
    sunny_m, shaded_m = split_sun_shadow_length(route_gdf.geometry, shadow_union)
    total_m = sunny_m + shaded_m
    shade_ratio = (shaded_m / total_m) if total_m > 0 else 0.0
    print(
        f"[Shortest] total={total_m:.1f} m, sunny={sunny_m:.1f} m, shaded={shaded_m:.1f} m, shade_ratio={shade_ratio:.1%}")
    # —— 新增结束 ——
    ...
    if new_route_gdf is not None:
        new_route_gdf.plot(ax=ax, color='green', linewidth=2, label='Wanted Bike Route')
        s_m, sh_m = split_sun_shadow_length(new_route_gdf.geometry, shadow_union)
        tot = s_m + sh_m
        print(
            f"[Wanted ] total={tot:.1f} m, sunny={s_m:.1f} m, shaded={sh_m:.1f} m, shade_ratio={(sh_m / tot) if tot > 0 else 0.0:.1%}")
    ...
    df.to_excel('route_length_analysis.xlsx', index=False)
#============================================================================================
